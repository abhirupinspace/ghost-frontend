...
export function SelectModal({ title, items, selectedId, onSelect, onClose }: SelectModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-[400px] bg-[#161619] rounded-2xl border border-[#1e1e24] overflow-hidden">
        {/* Header */}
        ...
        <div className="px-5 pb-3">
          <div className="flex items-center gap-2 px-3 py-2.5 bg-[#111114] rounded-xl border border-[#1e1e24]">
          ...
          {filtered.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors cursor-pointer ${
                item.id === selectedId ? "bg-[#1a1a1e]" : "hover:bg-[#111114]"
              }`}
            >
              ...
              {item.id === selectedId && (
                <div className="ml-auto w-2 h-2 rounded-full bg-[#e5e044]" />
              )}
            </button>
          ))}
          {filtered.length === 0 && (
